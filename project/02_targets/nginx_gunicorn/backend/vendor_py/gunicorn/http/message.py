...
# HDHunter internal-state instrumentation (B4b). Optional: absent on the
# coverage.py baseline image, so guard the import and every call site.
try:
...
class Message(object):
    ...
    def set_body_reader(self):
        ...
        if chunked:
            if _hdh is not None:
                _hdh.hdhunter_set_chunked_encoding(1, _hdh.MODE_REQUEST)
            ...
            if _hdh is not None:
                _hdh.hdhunter_set_content_length(content_length, _hdh.MODE_REQUEST)

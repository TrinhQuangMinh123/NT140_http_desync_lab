...
# HDHunter internal-state instrumentation (B4b). Optional — see message.py.
try:
...
class LengthReader(object):
    ...
    def read(self, size):
        ...
        self.length -= size
        # Content-Length body: bytes consumed off the wire == body bytes (paper
        # "Consumed" == "Body" for the CL case). This is the clean signal that
        # replaces the wire-length guess for rule-1/rule-7 (B6).
        if _hdh is not None and ret:
            _hdh.hdhunter_inc_consumed_length(len(ret), _hdh.MODE_REQUEST)
            _hdh.hdhunter_inc_body_length(len(ret), _hdh.MODE_REQUEST)
        return ret
